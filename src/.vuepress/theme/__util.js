...
export function layoutsFromNav(navs) {
  return navs.reduce((layouts, nav) => layouts.concat(nav.layout || []), []);
}

export function excludeFeature(page) {
  return (
    !page.frontmatter.date || +new Date(page.frontmatter.date) <= +new Date()
  );
}
...
export function pageWithCustomLayout(pages, navs, layout) {
  ...
  const pagesWithoutRoot = pages.filter(
    page =>
      page.path !== '/' && // exclude root
...
export function pageNormalize(pages, navs) {
  ...
  const navList = navs || [];
  const navLinks = navList.map(n => navsLinksNormalize(n.link));
  const withoutRoot = pages.filter(
    page =>
      page.path !== '/' &&
      excludeFeature(page) && // exclude page.date > current time
...
export function pageSortByDate(pages) {
  const pageWithDate = pages.filter(page => page.frontmatter.date);
  const pageWithOutDate = pages.filter(page => !page.frontmatter.date);

  let mapped = pageWithDate.map((p, i) => ({
    index: i,
